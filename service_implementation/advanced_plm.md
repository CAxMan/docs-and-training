...
The PLM Server has its own implementation of the GSS services, that is, PLM service endpoints can be obtained through GSS. GSS offers SOAP and REST services:
...
GSS covers a small subset of simplified methods (services) of the PLM Server. The remaining PLM Server functionality is only available as native PLM Server SOAP web-services; see below for details.

Java (Maven) client based examples can be retrieved from here: https://github.com/CAxMan/PlmGSS-samples

The PLM Server is also accessible via the GSS.
Code samples of that are here: https://github.com/CAxMan/infrastructureClients

The following GSS methods (SOAP) are only implemented for the PLM Server storage option, not for the SWIFT storage:
- *createFolderVersion*
- *listFolderVersions*
- *listFileVersions*
- *plm2gss*
...
Java (Maven) client based examples of how to apply these native services can be retrieved from here: https://github.com/CAxMan/plm_soap_samples
